...
export default function DietScreen() {
  const { colors, typography, spacing, radius } = useTheme()
  const { data: plan, isLoading, refetch } = trpc.diet.activePlan.useQuery()
        ...
        {currentDay && (
          <View style={{ padding: spacing.base, gap: spacing.md }}>
            ...
            {currentDay.meals.map((meal, i) => (
              <View key={i} style={{ backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.base, gap: spacing.sm }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontFamily: typography.family.semiBold, fontSize: typography.size.xs, color: colors.textMuted, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                      {meal.type}
                      {meal.isTreat ? ' ✨ treat' : ''}
                      {meal.batchCookable ? ' · 🍱 batch-friendly' : ''}
                    </Text>
