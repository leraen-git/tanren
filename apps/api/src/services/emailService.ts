...
const FROM_EMAIL = process.env['FROM_EMAIL'] ?? 'FitTrack <noreply@fittrack.app>'
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:480px;margin:0 auto;padding:40px 20px;color:#0E0E0E;background:#fff;">
  <h1 style="color:#E8192C;font-size:28px;margin:0 0 4px 0;letter-spacing:-0.5px;">FitTrack</h1>
  ...
  <div style="background:#F5F5F5;border-radius:16px;padding:32px;text-align:center;margin-bottom:24px;">
    <span style="font-size:44px;font-weight:800;letter-spacing:14px;color:#0E0E0E;">${code}</span>
  </div>
  <p style="color:#555;font-size:14px;line-height:22px;margin-bottom:32px;">
    Enter this code in the FitTrack app. It expires in <strong>10 minutes</strong> and can only be used once.
  </p>
...
export async function sendOtpEmail(to: string, code: string): Promise<void> {
  ...
  const { error } = await resend.emails.send({
    from: FROM_EMAIL,
    to: [to],
    subject: `Your FitTrack code: ${code}`,
    text: `Your FitTrack sign-in code is: ${code}\n\nThis code expires in 10 minutes. Do not share it with anyone.`,
